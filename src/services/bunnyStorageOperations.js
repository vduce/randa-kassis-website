...
export const createFile = async (categoryPath, filename, content) => {
  try {
    ...
    // If file exists (success = true), return error
    ...
    // If file doesn't exist (404 error), that's expected - proceed to create
    // Any other error should be handled
    if (existsCheck.error && existsCheck.error.code !== 'NOT_FOUND') {
      return existsCheck; // Return the error
    }
