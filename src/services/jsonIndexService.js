...
import { syncJSONToCDN, isCDNSyncAvailable } from './jsonCdnSync';
 ...
 * Also syncs to CDN if available
 */
export const writeJSONIndex = async (categoryKey, data) => {
  ...
  try {
    ...
    // Sync to CDN in background (don't wait for it)
    syncToCDNInBackground(categoryKey, data);
...
/**
 * Sync to CDN in background without blocking
 */
const syncToCDNInBackground = async (categoryKey, data) => {
  console.log(`🔄 Starting CDN sync for ${categoryKey}...`);
  try {
    console.log('   Checking if CDN sync is available...');
    const available = await isCDNSyncAvailable();
    console.log(`   CDN sync available: ${available}`);
    
    if (available) {
      console.log('   Calling syncJSONToCDN...');
      await syncJSONToCDN(categoryKey, data);
      console.log('   ✅ CDN sync completed');
    } else {
      console.log('⚠️ CDN sync skipped - proxy server not available');
    }
  } catch (error) {
    console.warn('❌ CDN sync failed (non-blocking):', error.message);
